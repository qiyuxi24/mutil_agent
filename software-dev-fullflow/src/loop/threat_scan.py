#!/usr/bin/env python3
"""Deterministic prompt-injection / promptware / exfiltration scanner.

The AI 研发团队 re-injects model- and web-authored content back into agent
context: MEMORY.md, skill docs, fetched web content, community-authored
SKILL.md. None of that is scanned by default. This module is the cheap,
deterministic FIRST layer of the two-layer defense:

  layer 1 (this file) — regex, fail-closed, no model. A poisoned model cannot
                        talk its way past a regex. Blocks overt injection.
  layer 2 (the review gate / acceptance gate) — cross-model review catches
                        semantic poisoning a regex can't (see
                        skills/review-gate, skills/evidence-integrity).
...
content still belongs to the cross-model review / acceptance gate.

Pattern set adapted from NousResearch/hermes-agent `tools/threat_patterns.py`
(MIT License, Copyright (c) 2025 Nous Research), via SJTU ARIS
(Auto-claude-code-research-in-sleep), with runtime adaptations (agent env
vars, skills/ install-list paths) and an added entry-level `quarantine()` +
CLI.
...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    # Runtime: unsetting known agent/provider env vars is pure attack behavior.
    ...
    # Config surface: tampering with the agent's instructions / install state.
    # Bounded gap + fixed-width negative lookbehind so "REVIEWER_MEMORY.md" does
    # NOT match via the "MEMORY.md" substring. NOTE: legit skill docs do say
    ...
    # skills/ install list — adapted from ARIS `.aris/installed-skills.txt`.
    (r'(update|modify|edit|write|change|append|add\s+to)\s+[^\n]{0,60}(?:skills/)?(?:SKILL-LIST|REGISTRY|ASSIGNMENT-MATRIX)\.md', "skill_registry_mod", "strict"),
...
def main() -> int:
    import argparse
    ap = argparse.ArgumentParser(description="Agent injection / exfiltration scanner.")
