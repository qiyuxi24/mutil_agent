...
# 一键启动「团队自建工具链」（阿里 AgentScope 实现）两个 REST 服务。
#
...
#   .\scripts\start-toolchains.ps1              # 启动两个服务
...
if ($Health) {
    foreach ($svc in @(@{n="code-scan";p=9100}, @{n="test-platform";p=9200})) {
        try {
...
Write-Host "`n两个工具链服务已启动。健康检查：.\\scripts\\start-toolchains.ps1 -Health" -ForegroundColor Green
Write-Host "注册到 Higress：.\\scripts\\register-toolchains.ps1" -ForegroundColor Green
