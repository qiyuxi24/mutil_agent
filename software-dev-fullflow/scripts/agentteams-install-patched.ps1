...
# [PATCH] Matrix AppService tokens (persisted so controller restarts are stable;
# omitted by upstream Windows installer, causing controller panic on startup)
AGENTTEAMS_MATRIX_APPSERVICE_ENABLED=$(if ($env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED) { $env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED } else { "true" })
AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN=$($env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN)
AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN=$($env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN)
...
function Install-Manager {
    ...
    # [PATCH] Generate Matrix AppService tokens BEFORE writing env file, so they
    # are persisted to disk (upstream Windows installer omitted this entirely,
    # causing agentteams-controller to panic: "AS_TOKEN is required").
    if (-not $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN) {
        $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN = [System.Guid]::NewGuid().ToString("N") + [System.Guid]::NewGuid().ToString("N")
    }
    if (-not $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN) {
        $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN = [System.Guid]::NewGuid().ToString("N") + [System.Guid]::NewGuid().ToString("N")
    }
    ...
    if ($script:AGENTTEAMS_MOUNT_SOCKET) {
        ...
        if ($socketAvailable) {
            # [PATCH] Guard against $ErrorActionPreference="Stop" turning the benign
            # "network not found" from `docker network inspect` into a terminating error.
    ...
    if ($script:AGENTTEAMS_USE_EMBEDDED -eq "1") {
        ...
        # ── [PATCH] Matrix AppService tokens ─────────────────────────────────
        # Windows installer omitted AS_TOKEN/HS_TOKEN generation + passthrough,
        # while agentteams-controller panics at startup if AppService mode is
        # enabled but these are empty. Align with agentteams-install.sh
        # (generates random hex tokens once, persists to env, passes to controller).
        # Ref: agentteams-controller/internal/config/config.go validate.
        $appSvcEnabled = if ($env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED) { $env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED } else { "true" }
        if ($appSvcEnabled -ne "false" -and $appSvcEnabled -ne "0") {
            if (-not $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN) {
                $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN = [System.Guid]::NewGuid().ToString("N") + [System.Guid]::NewGuid().ToString("N")
                Write-Log "  Auto-generated AppService as_token (saved to env file)"
            }
            if (-not $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN) {
                $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN = [System.Guid]::NewGuid().ToString("N") + [System.Guid]::NewGuid().ToString("N")
                Write-Log "  Auto-generated AppService hs_token (saved to env file)"
            }
        }
        $ctrlArgs += @("-e", "AGENTTEAMS_MATRIX_APPSERVICE_ENABLED=$appSvcEnabled")
        $ctrlArgs += @("-e", "AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN=$env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN")
        $ctrlArgs += @("-e", "AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN=$env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN")
        # ── [/PATCH] ─────────────────────────────────────────────────────────
