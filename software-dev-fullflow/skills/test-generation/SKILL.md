...
- 测试在**隔离沙箱环境**执行（Worker 自己的容器），**不触碰生产数据**、不访问宿主机与其他 Worker。
- 文件读写边界：只写当前工作区 + `shared/tasks/{id}/`；**禁止**读写 `.env`、`credentials/`、`providers.json` 等敏感文件。
- 测试执行设超时上限（默认 120s）；禁止在测试中执行生产写操作。
- 危险命令（删库/改权限/强制推送）禁止，或需 Manager 审批。
- 测试结果落盘为执行证据（`test-report.md` + `test.json`）可审计。
