...
- 默认独立分支操作，**不直接改主分支**。
- 代码执行（编译/类型检查/静态分析）发生在 Worker 自己的**沙箱容器**内，不触碰宿主机与其他 Worker。
- 文件读写边界：只写当前工作区 + `shared/tasks/{id}/`；**禁止**读写 `.env`、`credentials/`、`providers.json` 等敏感文件。
- 危险命令（`rm -rf /`、`chmod 777`、`git push --force`、删库）**禁止**，或需 Manager 审批。
- 编译/静态自检设超时上限（默认 60s）；失败重试最多 N 次后转 `PARTIAL_FAILED`。
- 凭据/密钥禁止硬编码；改动用 `git diff` 可审计。
